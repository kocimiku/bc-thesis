...
def check_slim_IP_output(slim_IP, slim_info, X, Y, coef_constraints):
    ...
    assert total_error <= min(slim_info['N_pos'],
                              slim_info['N_neg']), 'total_error should be less than total_error_pos + total_error_neg'
...
def get_rho_summary(rho, slim_info, X, Y):
    ...
    printed_model = print_slim_model(rho, X_names=slim_info['X_names'], Y_name=slim_info['Y_name'],
